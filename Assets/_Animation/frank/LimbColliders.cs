...
public class LimbColliders : MonoBehaviour {
	...
	public FrankBehavior fb;
	public float damagemult;
	// Use this for initialization

	void Start () {
		fb = GameObject.FindGameObjectWithTag ("frankcore").GetComponent<FrankBehavior> ();
	}

	public void OnCollisionEnter(Collision coll){
		print ("collisiontag: " + coll.gameObject.tag);
		if (coll.gameObject.tag == "manipulatable" && (coll.gameObject.transform.parent == null || coll.gameObject.transform.parent.position != fb.boulderparent.position)) {
		...
		if (coll.gameObject.tag == "sword") {
			...
			float dam = coll.gameObject.GetComponent<iteminfo> ().itemattack * (coll.relativeVelocity.magnitude/3f + .5f) * fb.pm.attack/2f;
